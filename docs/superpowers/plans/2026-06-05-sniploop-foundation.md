...
> **STATUS: COMPLETED (2026-06-05).** All 20 tasks are implemented and committed; `swift run SniploopCoreTests` is green and the app works end to end. Three environment-forced deviations were made and are documented inline: (1) the test harness is a stdlib assertion executable, not XCTest (no Xcode on the build machine); (2) the global hotkey is a direct Carbon `RegisterEventHotKey`, not the KeyboardShortcuts package (its `#Preview` macro needs Xcode); (3) `MenuBarController` subclasses `NSObject`. Post-plan additions made from real-use feedback (each their own commit, not in the task list below): a Settings window with a default **output format** (GIF/MP4/Both) so capture produces one format instead of always both; a persistent recording border; keyboard stop/cancel during recording (Return/Escape via transient Carbon hotkeys); high-contrast HUD chips for the overlay hints; and a stable self-signed dev signing identity so the Screen Recording grant survives rebuilds. The editor (M3) and remaining M4 work are the next plans.
...
**Testing Harness note (IMPORTANT, supersedes the XCTest snippets below):** This machine has Apple CommandLineTools but not Xcode, so `XCTest` and the `Testing` framework are unavailable and `swift test` cannot run. Tests are therefore a plain executable target, `SniploopCoreTests`, run with `swift run SniploopCoreTests` (exit 0 = green, non-zero = red). A shared `Sources/SniploopCoreTests/TestHarness.swift` provides `enum T` with `T.ok(_:_:)`, `T.eq(_:_:_:)`, `T.close(_:_:_:_:)`, and `T.finish()`. Each test file exposes one `func run<Name>Tests()` of `T` checks (using `import SniploopCore`, not `@testable`), and `Sources/SniploopCoreTests/main.swift` calls each `run...()` then `T.finish()`. The per-task code blocks below are written in XCTest form for readability; implement them as harness functions instead: `XCTAssertEqual(a, b)` becomes `T.eq(a, b, "label")`, `XCTAssertNil(x)` / `XCTAssertTrue(x)` / `XCTAssertFalse(x)` become `T.ok(...)`, float-accuracy asserts become `T.close(...)`, and add a `run...()` call to `main.swift`. The "red" step is `swift run SniploopCoreTests` failing to compile (symbol missing) or printing `FAIL`; "green" is `ALL PASSED`. Task 1 (already completed) established this harness; `SniploopCoreTests` is an `.executableTarget`, not a `.testTarget`, and the test files live under `Sources/SniploopCoreTests/`, not `Tests/`.
